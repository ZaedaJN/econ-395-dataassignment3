...
dhs_both_3$weight.ATE <- ifelse(dhs_both_3$Treatment == 1, 1/dhs_both_3$psvalue, 1/(1 - dhs_both_3$psvalue))

#step 6: outcomes analysis using propensity scores as weights in a weighted regression
q3.ATE <- suppressWarnings(
  glm(Treatment ~ resp_age + resp_edu + year_marr + religion + caste + husb_edu + hh_size +
        hhd_age + hhd_gender + place_residence + 
        poorest + poorer + middle + richer + 
        age_wi1 + age_wi2 + age_wi3 + age_wi4 +
        caste_husb +
...
varnames_3 <- q3_tidy$term

q3_tidy_renamed <- q3_tidy |> 
  mutate(term = labels) |>
  Treatment_by(Treatment)

##########
model_2 <- list(
  "weighted" = glm(Treatment ~ resp_age + resp_edu + year_marr + religion + caste + husb_edu + hh_size +
        hhd_age + hhd_gender + place_residence + 
        poorest + poorer + middle + richer + 
        age_wi1 + age_wi2 + age_wi3 + age_wi4 +
        caste_husb +
...
model_2_df <- tidy(model_2)

datasummary_balance(~ Treatment, model_2)

modelsummary(
  model_2,
  output = "kableExtra",
  title =  "Weighted Propensity Scores",
